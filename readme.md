...
- **Service Management**: Termux-friendly `/restart_extract_service` command that restarts the extract service asynchronously, surfaces script output, verifies running PIDs, and times out safely if the script hangs
...
5. **Available Commands:**
   ...
   - `/restart_extract_service` - Run the Termux restart script asynchronously with timeout handling and post-run status checks (the bot updates the original message when the restart finishes)
